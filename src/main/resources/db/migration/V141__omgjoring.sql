...
-- Tanken er at denne både kan være en enkeltperiode for å støtte krymping/utviding basert på endret tiltaksdeltagelsesperiode, og senere også periodiseres, slik at man støtter delvis innvilgelse med hull.
-- Vil kun brukes av omgjøring til å starte med.
ALTER TABLE behandling ADD COLUMN IF NOT EXISTS innvilgelsesperiode jsonb DEFAULT null;
ALTER TABLE behandling ADD COLUMN IF NOT EXISTS omgjør_rammevedtak_id varchar references rammevedtak(id) DEFAULT null;
ALTER TABLE rammevedtak ADD COLUMN IF NOT EXISTS omgjort_av_rammevedtak_id varchar references rammevedtak(id) DEFAULT null;
