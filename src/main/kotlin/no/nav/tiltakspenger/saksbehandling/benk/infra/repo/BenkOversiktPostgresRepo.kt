    ...
    private object BenkSpørringer {
        ...
        const val ÅPNE_TILBAKEKREVINGER = """
            select tb.sak_id              as sakId,
                ...
                case
                    when tb.status in ('TIL_FORHÅNDSVARSEL', 'TIL_BEHANDLING') and tb.saksbehandler_ident is not null then 'UNDER_BEHANDLING'
                    when tb.status in ('TIL_FORHÅNDSVARSEL', 'TIL_BEHANDLING') and tb.saksbehandler_ident is null then 'KLAR_TIL_BEHANDLING'
                ...
                case
                    when u.rammevedtak_id is not null then 'RAMMEVEDTAK'
                    when u.meldekortvedtak_id is not null then 'MELDEKORT'
                end                       as tilbakekrevingKilde
        ...
        const val ÅPNE_KLAGER = """
            select k.sak_id             as sakId,
                ...
                CASE
                  WHEN status = 'MOTTATT_FRA_KLAGEINSTANS'
    ...
    private fun Row.tilSammendrag(): BehandlingssammendragMedCount {
        ...
        return BehandlingssammendragMedCount(
            Behandlingssammendrag(
                ...
                resultat = resultat,
                beløp = beløp,
                tilbakekrevingKilde = tilbakekrevingKilde,
            ),
            totalAntall = count,
