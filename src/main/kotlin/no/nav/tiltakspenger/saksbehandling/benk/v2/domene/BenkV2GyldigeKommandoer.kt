...
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkTilbakekrevingStatus.OPPRETTET
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkTilbakekrevingStatus.TIL_BEHANDLING
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkTilbakekrevingStatus.TIL_FORHÅNDSVARSEL
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkTilbakekrevingStatus.TIL_GODKJENNING
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkTilbakekrevingStatus.UNDER_BEHANDLING
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkTilbakekrevingStatus.UNDER_FORHÅNDSVARSLING
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkTilbakekrevingStatus.UNDER_GODKJENNING
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkV2Behandlingsstatus.KLAR_TIL_BEHANDLING
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkV2Behandlingsstatus.KLAR_TIL_BESLUTNING
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkV2Behandlingsstatus.UNDER_AUTOMATISK_BEHANDLING
import no.nav.tiltakspenger.saksbehandling.benk.v2.domene.BenkV2Behandlingsstatus.UNDER_BESLUTNING
...
private fun finnGyldigeRammebehandlingKommandoer(
    ...
    return buildList {
        ...
        if (status == BenkV2Behandlingsstatus.UNDER_BEHANDLING && felles.saksbehandler != navIdent && erSaksbehandler && felles.saksbehandler != null) {
            add(OvertaSaksbehandler)
        }
        if (status == UNDER_BESLUTNING && felles.beslutter != navIdent && erBeslutter && felles.beslutter != null && felles.saksbehandler != navIdent) {
            add(OvertaBeslutter)
        }
        if (status == BenkV2Behandlingsstatus.UNDER_BEHANDLING && erSaksbehandler && felles.saksbehandler == navIdent) {
            add(LeggTilbakeSaksbehandler)
        }
        ...
        if (!erSattPåVent && when (status) {
                ...
                BenkV2Behandlingsstatus.UNDER_BEHANDLING -> erSaksbehandler && felles.saksbehandler == navIdent
        ...
        if (erSattPåVent && when (status) {
                KLAR_TIL_BEHANDLING, BenkV2Behandlingsstatus.UNDER_BEHANDLING, UNDER_AUTOMATISK_BEHANDLING -> erSaksbehandler
        ...
        if (when (status) {
                UNDER_AUTOMATISK_BEHANDLING, KLAR_TIL_BEHANDLING, BenkV2Behandlingsstatus.UNDER_BEHANDLING -> true
...
private fun finnGyldigeMeldekortKommandoer(
    ...
    return buildList {
        ...
        if (when (status) {
                BenkV2Behandlingsstatus.UNDER_BEHANDLING, KLAR_TIL_BESLUTNING -> felles.saksbehandler == navIdent
                UNDER_BESLUTNING -> felles.beslutter == navIdent
                else -> false
            }
        ) {
