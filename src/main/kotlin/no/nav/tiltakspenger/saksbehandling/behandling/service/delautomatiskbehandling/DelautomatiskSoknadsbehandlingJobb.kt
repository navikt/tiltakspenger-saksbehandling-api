...
class DelautomatiskSoknadsbehandlingJobb(
    ...
    suspend fun opprettBehandlingForNyeSoknader() {
        ...
        ubehandledeSoknader.forEach {
            try {
                log.info { "Oppretter automatisk behandling for søknad med id ${it.id}" }
                ...
                log.error(e) { "Noe gikk galt ved oppretting av automatisk behandling for søknad med id ${it.id}" }
            }
        }
    }
    ...
    suspend fun behandleSoknaderAutomatisk() {
        ...
        automatiskeBehandlinger.forEach {
            try {
                log.info { "Starter behandling med id ${it.id} for sknad med id ${it.søknad.id}" }
                val correlationId = CorrelationId.generate()
                delautomatiskBehandlingService.behandleAutomatisk(it, correlationId)
                log.info { "Ferdig med å behandle søknad med id ${it.søknad.id} og behandlingsid ${it.id}" }
            } catch (e: Exception) {
                log.error(e) { "Noe gikk galt ved automatisk behandling av behandling med id ${it.id}" }
            }
        }
    }
}
