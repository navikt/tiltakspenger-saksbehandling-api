...
fun Sak.sendFørstegangsbehandlingTilBeslutning(
    ...
    clock: Clock,
): Either<no.nav.tiltakspenger.saksbehandling.behandling.domene.behandling.KanIkkeSendeTilBeslutter, Pair<Sak, Behandling>> {
    if (!kommando.saksbehandler.erSaksbehandler()) {
        return no.nav.tiltakspenger.saksbehandling.behandling.domene.behandling.KanIkkeSendeTilBeslutter.MåVæreSaksbehandler.left()
    }
    ...
    if (overlapperEllerTilstøterNyInnvilgelsesperiodeMedEksisterende(behandling.id, kommando.innvilgelsesperiode)) {
        return no.nav.tiltakspenger.saksbehandling.behandling.domene.behandling.KanIkkeSendeTilBeslutter.PeriodenOverlapperEllerTilstøterMedAnnenBehandling.left()
    }
