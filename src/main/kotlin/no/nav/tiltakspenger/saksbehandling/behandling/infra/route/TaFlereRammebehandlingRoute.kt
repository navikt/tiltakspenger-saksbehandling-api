...
data class TaFlereRammebehandlinger(
    val behandlingIder: List<String>,
)
...
fun Route.taFlereRammebehandlingerRoute(
    ...
    post(TA_FLERE_BEHANDLING_PATH) {
        ...
        call.withBody<TaFlereRammebehandlinger> { behandlingIder ->

            val behandlingIder = behandlingIder.behandlingIder.map {
                RammebehandlingId.fromString(it)
            }

            val correlationId = call.correlationId()
            krevSaksbehandlerEllerBeslutterRolle(saksbehandler)

            taBehandlingService.hentSakIder(behandlingIder = behandlingIder)
