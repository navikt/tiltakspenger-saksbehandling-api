...
import no.nav.tiltakspenger.saksbehandling.behandling.domene.Behandlingstype.SØKNADSBEHANDLING
...
private data class BehandlingGammel(
     ...
     */
    val utfall: BehandlingsutfallGammel?, // alle behandlinger (på litt annen måte)
    ...
    val maksDagerMedTiltakspengerForPeriode: Int = MAKS_DAGER_MED_TILTAKSPENGER_FOR_PERIODE_OLD
    ...
    val utfallsperioder: Periodisering<Utfallsperiode>? by lazy {
        ...
        when (behandlingstype) {
            SØKNADSBEHANDLING -> Periodisering(Utfallsperiode.RETT_TIL_TILTAKSPENGER, virkningsperiode)
    ...
    companion object {
        ...
        const val MAKS_DAGER_MED_TILTAKSPENGER_FOR_PERIODE_OLD: Int = 10
        ...
        suspend fun opprettSøknadsbehandling(
            ...
            clock: Clock,
        ): Either<KanIkkeOppretteBehandling, BehandlingGammel> {
            ...
            return BehandlingGammel(
                ...
                avbrutt = null,
                antallDagerPerMeldeperiode = MAKS_DAGER_MED_TILTAKSPENGER_FOR_PERIODE_OLD,
                avslagsgrunner = null,
                utfall = null,
            ).right()
        }
        ...
        suspend fun opprettRevurdering(
            ...
            return BehandlingGammel(
                ...
                utfall = BehandlingsutfallGammel.STANS,
            )
        }
    }

    /** Saksbehandler/beslutter tar eller overtar behandlingen. */
    fun taBehandling(saksbehandler: Saksbehandler): BehandlingGammel {
    ...
    fun overta(saksbehandler: Saksbehandler, clock: Clock): Either<KunneIkkeOvertaBehandling, BehandlingGammel> {
        return when (this.status) {
    ...
    fun leggTilbakeBehandling(saksbehandler: Saksbehandler): Either<KanIkkeLeggeTilbakeBehandling, BehandlingGammel> {
        return when (this.status) {
...
//            utfall = kommando.utfall,
            ...
            valgteTiltaksdeltakelser = null, // kommando.valgteTiltaksdeltakelser(this),
     ...
     * Endrer ikke [Søknad].
     */
    fun krymp(nyPeriode: Periode): BehandlingGammel {
    ...
    fun oppdaterBarnetillegg(kommando: OppdaterBarnetilleggKommando): BehandlingGammel {
    ...
    fun avbryt(avbruttAv: Saksbehandler, begrunnelse: String, tidspunkt: LocalDateTime): BehandlingGammel {
