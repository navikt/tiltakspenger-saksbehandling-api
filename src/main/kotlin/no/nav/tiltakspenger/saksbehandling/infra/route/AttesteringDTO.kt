...
fun List<Attestering>.toAttesteringDTO(): List<AttesteringDTO> {
    return this.map { it.toAttesteringDTO() }
}

fun Attestering.toAttesteringDTO() = AttesteringDTO(
    endretAv = this.beslutter,
    status = this.status,
    begrunnelse = this.begrunnelse?.value.ikkeSladdet(),
