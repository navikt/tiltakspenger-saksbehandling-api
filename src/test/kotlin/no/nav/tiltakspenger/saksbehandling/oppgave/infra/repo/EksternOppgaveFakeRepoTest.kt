...
class EksternOppgaveFakeRepoTest {
    ...
    fun `første registrering beholdes også når et senere duplikat har annen sak og grunnlag`() {
        ...
        repo.hentForSakId(førsteSak) shouldBe listOf(første.lagret())
    ...
    fun `filtrerer på sak og sorterer på tidspunkt og oppgave-id`() {
        ...
        repo.hentForSakId(sakId) shouldBe listOf(eldste, førsteVedLikTid, sisteVedLikTid).map { it.lagret() }
        repo.hentForSakId(annenSak.sakId) shouldBe listOf(annenSak.lagret())
    }

    private fun EksternOppgave.lagret() = LagretEksternOppgave(
        oppgaveId = oppgaveId,
        sakId = sakId,
        opprettet = opprettet,
        grunnlag = grunnlag.toDbJson(),
