...
class OppdaterValgtHjemmelService(
    private val sakService: SakService,
    private val behandlingRepo: BehandlingRepo,
) {
    suspend fun oppdaterValgtHjemmel(
        sakId: SakId,
        ...
        val sak = sakService.hentForSakId(sakId, saksbehandler, correlationId).getOrElse {
            throw IllegalStateException("Kunne ikke oppdatere valgt hjemmel. Fant ikke sak. sakId=$sakId, behandlingId=$behandlingId")
        }
        val behandling = sak.hentBehandling(behandlingId)!!

        return behandling.oppdaterValgtHjemmelHarIkkeRettighet(saksbehandler, valgtHjemmelHarIkkeRettighet).also {
